// A demo of various aspects of blending model with ordering of objects (solution)
'use strict';
...
function initProgram() {
    ...
    let vert_shader = compileShader(gl, gl.VERTEX_SHADER,
        ...
        void main() {
            ...
            vec4 lightPos = /*ViewMatrix **/ light;
...
function onMouseDown(e) {
    ...
    function onMouseMove(e2) {
        ...
        glMatrix.mat4.fromRotationTranslation(view_matrix,
            glMatrix.quat.fromEuler(glMatrix.quat.create(), ...rotation), [0, 0, -5]);
        gl.uniformMatrix4fv(gl.program.uViewMatrix, false, view_matrix);
    }
    function onMouseUp() {
...
function render() {
    ...
    // Assume the center of every object is 0,0,0 before the model and view matrices are applied.
    // Create an array of arrays. Each sub-array has the z value of the object (after being
    // transformed by the individual model and global view matrices) and  the index of the object.
    let depths = [];
    for (let i = 0; i < objs.length; i++) {
        let [vao, count, mode, name, model_matrix] = objs[i];
        let coord = vec3.transformMat4(vec3.create(), [0, 0, 0], model_matrix);
        vec3.transformMat4(coord, coord, view_matrix);
        depths.push([coord[2], i]);
    }

    // Sort by depth and extract the render order from it
    depths.sort((a, b) => { return a[0] - b[0]; });
    let render_order = depths.map(x => x[1]);

    // Render all of the objects in the determined order
