// A demo of various aspects of blending model
'use strict';
...
function initProgram() {
    ...
    let vert_shader = compileShader(gl, gl.VERTEX_SHADER,
        ...
        void main() {
            mat4 mv = uModelMatrix * uViewMatrix;
            vec4 P = mv * aPosition;
...
function initBuffers() {
    ...
    objs[1].push('tetra');
    objs[1].push(mat4.fromTranslation(mat4.create(), [1, 0, 0.5]));
}
...
function createObject(coords, indices, is_tri_strip) {
    ...
    let normals = calc_normals(coords, indices, is_tri_strip);
...
function initEvents() {
    ...
    // No events for the color/alpha boxes - they are re-read every render
}
...
function onMouseDown(e) {
    ...
    function onMouseMove(e2) {
        ...
        let mv = glMatrix.mat4.fromRotationTranslation(glMatrix.mat4.create(),
...
function updateBlending() {
    if (document.getElementById('blending').checked) {
        let s = gl[document.getElementById('src-factor').value];
        let d = gl[document.getElementById('dst-factor').value];
        // TODO
    } else {
        gl.disable(gl.BLEND);
    }
...
function render() {
    ...
    for (let [vao, count, mode, name, model_matrix] of objs) {
        gl.bindVertexArray(vao);
