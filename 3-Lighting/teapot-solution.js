...
function calc_normals(positions, indices) {
    ...
    for (let i = 0; i < indices.length - 2; i += 3) {
        ...
        glMatrix.vec3.cross(N_face,
            glMatrix.vec3.subtract(V, A, B),
            glMatrix.vec3.subtract(U, A, C));
        glMatrix.vec3.add(NA, NA, N_face);
        glMatrix.vec3.add(NB, NB, N_face);
        glMatrix.vec3.add(NC, NC, N_face);
    }
