// A basic demo of using multiple textures that are drawn side-by-side (solution)
'use strict';
...
window.addEventListener('load', function init() {
    ...
    gl.uniform1i(gl.program.uTexture1, 0);
    gl.uniform1i(gl.program.uTexture2, 1);
...
function initProgram() {
    ...
    let frag_shader = compileShader(gl, gl.FRAGMENT_SHADER,
        ...
        void main() {
            ...
            vec4 color;
            if (vTexCoord.s < 0.5) {
                vec2 texCoord = vec2(vTexCoord.s * 2.0, vTexCoord.t);
                color = texture(uTexture1, texCoord);
            } else {
                vec2 texCoord = vec2((vTexCoord.s - 0.5) * 2.0, vTexCoord.t);
                color = texture(uTexture2, texCoord);
            }
    ...
    program.uTexture1 = gl.getUniformLocation(program, 'uTexture1');
    program.uTexture2 = gl.getUniformLocation(program, 'uTexture2');
